...
if [ -n "$NGROK_AUTH" ]; then
  cp $HOME/.ngrok2/ngrok.yml $HOME/.ngrok2/ngrok.yml.original
  (grep -vF 'authtoken:' $HOME/.ngrok2/ngrok.yml.original; echo -e "\nauthtoken: $NGROK_AUTH") > $HOME/.ngrok2/ngrok.yml
fi
...
elif [ -n "$NGROK_DOMAIN" ] && [ -n "$NGROK_AUTH" ]; then
  ARGS="$ARGS --domain=$NGROK_DOMAIN "
